...
export default function FeedbackD() {
  ...
  const handleSendData = async () => {
    ...
    try {
      const response = await fetch("https://pythonmosiai.azurewebsites.net/feedback_analysis", {
        method: "POST",
        headers: {
      ...
      setResult(result.analysis); // Update according to the expected response structure
      Swal.close();
    } catch (error) {
  ...
  return (
    <AnimatedPage>
       <h1 class="text-3xl font-bold text-gray-900 mb-4">AI Feedback Anaylsis</h1>
<p class="text-lg text-gray-700 mb-8">Upload your csv of your customer  feedback from google play , survey anywhere and lets do the hard part</p>
